...
export async function applyMissionControlAction(form: FormData) {
  ...
  if (intent === "start")
    senderos.missionControl.startGoal({ goalId: String(form.get("goalId")) });
  if (intent === "retry")
    senderos.missionControl.retryExecution({
      goalId: String(form.get("goalId")),
    });
  if (intent === "stop")
    senderos.missionControl.stopExecution({ runId: String(form.get("runId")) });
  if (intent === "evidence")
