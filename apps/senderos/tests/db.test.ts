...
import { localSqliteAdapter } from '../src/adapters/db/local-sqlite';
import {
  describeCurrentDb,
...
import { mapFeatureRow, mapTaskRow } from '../src/db/mappers';
import { migrate } from '../src/db/schema';
import { emitEvent } from '../src/services/events';
import { initHome, tempHome, tursoConfigForHome } from './helpers/runtime';
...
describe('database adapters and helpers', () => {
  ...
  test('describes, validates, and rejects direct command execution for turso runtimes', async () => {
    ...
    const { initializeRuntime } = await import('../src/config/runtime');
    initializeRuntime(home, tursoConfigForHome(home));

    const { tursoAdapter } = await import('../src/adapters/db/turso');
