...
describe('cli help metadata', () => {
  test('resolves top-level and subcommand help with agent descriptions by default', () => {
    ...
    expect(resolveHelp('agent').subcommands?.map((x) => x.command)).toContain(
      'list',
    );
    ...
    expect(resolveHelp('config', 'set').arguments?.map((x) => x.name)).toEqual([
      'config-path',
      'value',
    ]);
  ...
  test('can omit agent descriptions from help output', () => {
    expect(
      resolveHelp('run', 'dispatch', { omitAgentDescription: true })
        .agentDescription,
    ).toBeUndefined();
  });
});
