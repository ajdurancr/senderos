...
export type FeatureStatus =
  ...
  | 'canceled'
  | 'completed';
export type SenderoStep = 'idle' | 'implementation' | 'review' | 'mutation' | 'done' | 'blocked';
...
export interface FeatureRecord { id:string; projectId:string; title:string; specText:string; sourceRequestText:string; gherkinText:string; gherkinMetaJson:string; status:FeatureStatus; senderoStep:SenderoStep; baseTargetBranch:string; featureBranchName:string|null; prUrl:string|null; prNumber:number|null; currentWorkspaceId:string|null; currentRunId:string|null; createdAt:string; updatedAt:string; }
export interface TaskRecord { id:string; featureId:string; name:string; phase:SenderoStep; status:TaskStatus; instructionJson:string; resultJson:string; createdAt:string; updatedAt:string; }
