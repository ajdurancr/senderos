...
import { cleanupWorkspace, ensurePhaseTask } from '../runtime/run';
...
export function approveFeature(id: string, home?: string) {
  ...
  const db = openRuntimeDb(home);
  db.prepare('update features set status=?, sendero_step=?, updated_at=? where id=?').run(
    'active',
    'idle',
    now(),
    id
...
export function cancelFeature(id: string, home?: string) {
  ...
  db.prepare('update features set status=?, sendero_step=?, current_run_id=?, updated_at=? where id=?').run(
    'canceled',
    'blocked',
    null,
    now(),
