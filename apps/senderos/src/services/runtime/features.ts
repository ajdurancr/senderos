...
export function listFeatures(home?: string): FeatureRecord[] { const db = openDb(home); const rows = db.query("select * from features order by created_at asc").all().map(mapFeatureRow) as FeatureRecord[]; db.close(); return rows; }
export function getFeature(id: string, home?: string): FeatureRecord | null { const db = openDb(home); const row = mapFeatureRow(db.query("select * from features where id = ?").get(id)); db.close(); return row; }
...
export function approveFeature(id: string, home?: string) {
  ...
  const db = openDb(home); db.prepare("update features set status=?, loop_phase=?, updated_at=? where id=?").run("ready_contract", "contract", now(), id); emitEvent(db, "feature.approved", "feature", id, {}); db.close(); ensurePhaseTask(getFeature(id, home)!, "contract", home); return getFeature(id, home);
}
export function cancelFeature(id: string, home?: string) { const db = openDb(home); db.prepare("update features set status=?, updated_at=? where id=?").run("canceled", now(), id); db.prepare("update tasks set status='canceled', updated_at=? where feature_id=? and status not in ('completed','failed')").run(now(), id); emitEvent(db, "feature.canceled", "feature", id, {}); db.close(); return getFeature(id, home); }
