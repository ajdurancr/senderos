...
export function getTask(id: string, home?: string) { const db = openDb(home); const row = mapTaskRow(db.query("select * from tasks where id=?").get(id)); db.close(); return row; }
export function listTasks(featureId: string, home?: string) { const db = openDb(home); const rows = db.query("select * from tasks where feature_id=? order by created_at asc").all(featureId).map(mapTaskRow); db.close(); return rows.filter(Boolean); }
