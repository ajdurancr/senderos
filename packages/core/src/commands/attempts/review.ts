...
export async function reviewRunAttempt(input: { attemptId: string; status: ReviewStatus; reviewer: string; rationale?: string; home?: string }) {
  ...
  await updateRunAttempt(input.attemptId, { statusSnapshot: { ...snapshot, review } }, input.home);
  const db = openRuntimeDb(input.home);
  await emitEvent(db, 'run-attempt.reviewed', 'run-attempt', input.attemptId, review);
  return review;
}
