...
export async function createRunRecord(goal: GoalRecord, home?: string) {
  ...
  const ts = now();
  await db.insert(runs).values({ id, goalId: goal.id, status: 'executing', branchName: goal.branchName, baseBranch: goal.baseTargetBranch, maxAttempts: 3, createdAt: ts, updatedAt: ts });
