...
export async function createProject(input: {
  ...
  await db
    ...
    .values({
      ...
      integrationMode: input.integrationMode ?? "github_pr",
      inferredCommandsJson: JSON.stringify(input.inferredCommands ?? {}),
      healthDetailsJson: JSON.stringify(input.healthDetails ?? {}),
      createdAt: ts,
      updatedAt: ts,
    });
