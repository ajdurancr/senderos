...
export async function cancelGoal(id: string, home?: string) {
  ...
  const ts = now();
  const goalRuns = await db
    .select({ id: runs.id })
    .from(runs)
    .where(eq(runs.goalId, id));
  await db
    .update(goals)
    .set({ status: "canceled", updatedAt: ts })
  ...
  if (goalRuns.length)
    await db
      .update(runAttempts)
      .set({ status: "canceled", finishedAt: ts, updatedAt: ts })
      .where(
        and(
          inArray(
            runAttempts.runId,
            goalRuns.map((run) => run.id),
          ),
          inArray(runAttempts.status, ["queued", "running", "paused"]),
        ),
      );
