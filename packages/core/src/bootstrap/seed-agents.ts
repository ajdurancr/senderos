...
async function ensureDefaultTransition(
  ...
  const existing = (
    await db
      ...
      .where(
        and(
          eq(agentTransitions.sourceAgentId, agent.id),
          eq(agentTransitions.name, transitionName),
        ),
      )
  )[0] as AgentTransitionRecord | undefined;
...
export async function seedBuiltInAgents(
  ...
  for (const entry of readdirSync(definitionsDir, { withFileTypes: true })) {
    ...
    const existing = (
      await db.select().from(agents).where(eq(agents.slug, seed.slug))
    )[0] as AgentRecord | undefined;
    if (existing)
      await db
        .update(agents)
        .set({
          ...
          kind: seed.kind,
          status: seed.status,
          definitionFormat: seed.definitionFormat,
          definitionBody: seed.definitionBody,
          defaultGoal: seed.defaultGoal,
          defaultMetaJson: seed.defaultMetaJson,
          updatedAt: ts,
        })
        .where(eq(agents.id, existing.id));
    else await db.insert(agents).values(seed);
    const agent = (
      await db.select().from(agents).where(eq(agents.slug, seed.slug))
    )[0] as AgentRecord;
    await ensureDefaultTransition(db, agent);
    seeded.push(agent);
    await emitEvent(db, "agent.seeded", "agent", agent.id, {
      slug: agent.slug,
    });
  }
  return seeded;
