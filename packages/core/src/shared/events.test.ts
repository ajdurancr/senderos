...
describe("emitEvent", () => {
  test("writes an event record", async () => {
    ...
    await emitEvent(db, "feature.created", "feature", "feature-1", {
      ok: true,
    });
...
test("listEvents filters persisted runtime events", async () => {
  ...
  expect((await listEvents({ home, entityId: "goal-1" }))[0]?.entityId).toBe(
    "goal-1",
  );
  expect(
    await listEvents({ home, entityType: "goal", entityId: "goal-1" }),
  ).toHaveLength(1);
  db.$client.close();
});
